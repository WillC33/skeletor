...
class BackgroundManager {
  ...
  init() {
    ...
    browser.commands.onCommand.addListener(command => {
      if (command === 'insert-template') {
        this.handleTemplateCommand()
      }
    })
  ...
  async handleTemplateCommand() {
    ...
    if (tabs[0]) {
      browser.tabs.sendMessage(tabs[0].id, { action: 'triggerTemplateSelector' })
        .catch(err => console.error('Failed to trigger template selector:', err))
    }
  }
      ...
      switch (message.action) {
        ...
        case 'getShortcut':
          try {
            ...
            const insertCommand = commands.find(cmd => cmd.name === 'insert-template')
            sendResponse({
              shortcut: insertCommand?.shortcut || 'Ctrl+Alt+I',
              description: insertCommand?.description || 'Open template selector'
            })
          } catch (error) {
